 ...
 */
function collectd_draw_rrd($host, $plugin, $pinst = NULL, $type, $tinst = NULL, $opts = [])
{
    global $config;
    $timespan_def = NULL;
    ...
    $rrd_cmd = ['-', '-E', '-a', 'PNG', '-w', $config['rrd_width'], '-h', $config['rrd_height']];

    if ($config['rrd_width'] <= "300") {
        $small_opts = ['--font', "LEGEND:7:mono", '--font', "AXIS:6:mono", "--font-render-mode", "normal"];
...
#                    '-t', $opts['title']);
    ...
    $cmd = ['-', '-E', '-a', 'PNG', '-w', $config['rrd_width'], '-h', $config['rrd_height']];

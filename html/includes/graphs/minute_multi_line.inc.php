...
foreach ($rrd_list as $i => $rrd) {
    ...
    if (in_array("lst", $data_show)) {
        $rrd_optionsb .= " GPRINT:" . $id . "m:LAST:%6.1lf%s";
    }
    if (in_array("min", $data_show)) {
        $rrd_optionsb .= " GPRINT:" . $id . "mmin:MIN:%6.1lf%s";
    }
    if (in_array("max", $data_show)) {
        $rrd_optionsb .= " GPRINT:" . $id . "mmax:MAX:%6.1lf%s";
    }
    if (in_array("avg", $data_show)) {
        $rrd_optionsb .= " GPRINT:" . $id . "m:AVERAGE:%6.1lf%s";
    }

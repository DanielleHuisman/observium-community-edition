...
if (is_numeric($vars['topn']) && $vars['topn'] > 0 && $vars['topn'] < 1024) {
    $topn = $vars['topn'];
} else {
    $topn = '10';
}
...
if (isset($sort)) {
    ...
    $mas = dbFetchRows($query, [$port['port_id'], $sort, $topn]);
}
...
foreach ($mas as $ma) {
    ...
    if (rrd_is_file($this_rrd)) {
        ...
        $name = $mac;
        $addy = dbFetchRow("SELECT * FROM `ip_mac` WHERE `mac_address` = ? AND `port_id` = ?", [$ma['mac'], $ma['port_id']]);

        if ($addy) {
            $name = $addy['ipv4_address'] . " (" . $mac . ")";
            $peer = dbFetchRow("SELECT * FROM ipv4_addresses AS A, ports AS I, devices AS D
              WHERE A.ipv4_address = ? AND I.port_id = A.port_id AND D.device_id = I.device_id", [$addy['ipv4_address']]);
            if ($peer) {
            ...
            if (dbExist('bgpPeers', '`device_id` = ? AND `bgpPeerIdentifier` = ?', [$ma['device_id'], $addy['ipv4_address']])) {
                $peer_info = dbFetchRow("SELECT * FROM bgpPeers WHERE device_id = ? AND bgpPeerIdentifier = ?", [$ma['device_id'], $addy['ipv4_address']]);

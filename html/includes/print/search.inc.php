    ...
    if ($data['type'] === 'rows') {
        ...
        if (str_contains_array($base_class, ['widget', 'box'])) {
            ...
            $box_args = [
              'id'            => 'box-' . $form_id,
              'header-border' => TRUE,
              'body-style'    => 'padding-top: ' . $base_space . ' !important;'
            ];
...
SCRIPT;
        ...
        case 'switch-new':
            ...
            foreach ($item_attribs as $attr) {
                if (isset($item[$attr])) {
                    $item['attribs']['data-' . $attr] = $item[$attr];
                }
            }
            // Append icons
            if (isset($item['icon'])) {
        ...
        case 'datetime':
            ...
            if ($item['presets']) {
                $presets = ['sixhours'  => 'Last 6 hours',
                            ...
                            'lyear'     => 'Last year'];
                // Recursive call
                $preset_item = ['id'     => $item['id'] . '_preset',
                                'type'   => 'select',
                                'name'   => 'Date presets',
                                'width'  => '110px',
                                'values' => $presets];
    ...
    switch ($type) {
        ...
        default:
            ...
            if (function_exists($form_function)) {
                return call_user_func_array($form_function, [$form_filter, $column, $options]);
            }
    }

...
function attach_click_events() {
    ...
    jQuery('#link_pick').click(link_picker).attr("href", "#");
    jQuery('#node_pick').click(node_picker).attr("href", "#");
...
function link_picker() {

    //this.preventDefault();

    // make sure it isn't already opened
    if (!newWindow || newWindow.closed) {
        newWindow = window.open("", "picker", "scrollbars=1,status=1,height=400,width=400,resizable=1");
...
function coord_update(event) {
    ...
    jQuery('#tb_coords').html('Position ' + cursorx + ', ' + cursory);
}

function coord_release(event) {
    jQuery('#tb_coords').html('Position ----, ---');
}

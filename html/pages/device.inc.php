...
if (!device_permitted($vars['device']) && safe_empty($permit_tabs)) {
...
} elseif (!$device = device_by_id_cache($vars['device'])) {
    ...
    if (!device_permitted($device['device_id'])) {
        ...
        $health_exist = [
          ...
          'sensors'    => dbExist('sensors', '`device_id` = ? AND `sensor_deleted` = ?', [$device['device_id'], 0]),
          'status'     => dbExist('status', '`device_id` = ? AND `status_deleted` = ?', [$device['device_id'], 0]),
          'counter'    => dbExist('counters', '`device_id` = ? AND `counter_deleted` = ?', [$device['device_id'], 0]),
        ];
        //r($health_exist);
        ...
        if ($health) {
            $navbar['options']['health'] = ['text' => 'Health', 'icon' => $config['icon']['health']];
        }
        ...
        $device_routing_count['ospf'] = dbFetchCell("SELECT COUNT(*) FROM `ospf_instances` WHERE `device_id` = ?", [$device['device_id']]);
        ...
        if ($_SESSION['userlevel'] >= 7) {
            ...
            if ($showconfig) {
                $navbar['options']['showconfig'] = ['text' => 'Config', 'icon' => $config['icon']['config']];
            }
        }

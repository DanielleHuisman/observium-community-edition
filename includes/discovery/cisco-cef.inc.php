...
$cefs = snmpwalk_cache_threepart_oid($device, "cefSwitchingPath", [], 'CISCO-CEF-MIB');
if (OBS_DEBUG > 1) {
    print_vars($cefs);
}
    ...
    foreach ($cefs as $entity => $afis) {
        ...
        foreach ($afis as $afi => $paths) {
            ...
            foreach ($paths as $path => $path_name) {
                ...
                if (!dbExist('cef_switching', '`device_id` = ? AND `entPhysicalIndex` = ? AND `afi` = ? AND `cef_path` = ?', [$device['device_id'], $entity, $afi, $path])) {

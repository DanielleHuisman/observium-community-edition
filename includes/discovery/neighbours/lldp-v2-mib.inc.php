...
$lldpRemTable_oids = ['lldpV2RemChassisIdSubtype', 'lldpV2RemChassisId', 'lldpV2RemPortIdSubtype',
                      'lldpV2RemPortId', 'lldpV2RemPortDesc', 'lldpV2RemSysName'];
$lldp_array        = [];
foreach ($lldpRemTable_oids as $oid) {
    $lldp_array = snmpwalk_cache_oid($device, $oid, $lldp_array, "LLDP-V2-MIB");

    if (empty($lldp_array)) {
        break;
    } // Stop walk if no data
}
    ...
    */
    ...
    foreach ($lldp_addr as $index => $entry) {
        ...
        */
        ...
        // LLDP index
        $lldpV2RemTimeMark            = array_shift($index_array);
        $lldpV2RemLocalIfIndex        = array_shift($index_array);
        $lldpV2RemLocalDestMACAddress = array_shift($index_array);
        $lldpV2RemIndex               = array_shift($index_array);
        ...
        $entry['lldpV2RemManAddrIndex'] = array_shift($index_array);

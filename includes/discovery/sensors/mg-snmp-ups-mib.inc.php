...
if (safe_count(array_keys($cache['mge'])) > $numPhase) {
    unset($cache['mge'][0]);
...
foreach ($cache['mge'] as $index => $entry) {
    ...
    discover_sensor('load', $device, $oid, "mgoutputLoadPerPhase.$index", 'mge-ups', $descr . ' Load', 1, $value, ['limit_high' => 85, 'limit_high_warn' => 70]);
...
$cache['mge'] = [];
foreach (["upsmgConfigEnvironmentTable", "upsmgEnvironmentSensorTable"] as $table) {
    echo("$table ");
    $cache['mge'] = snmpwalk_cache_oid($device, $table, $cache['mge'], "MG-SNMP-UPS-MIB", NULL, OBS_SNMP_ALL_NUMERIC_INDEX);
}

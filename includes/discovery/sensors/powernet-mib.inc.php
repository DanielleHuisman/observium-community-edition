...
if ($inputs || $outputs) {
  ...
  for ($o = 1; $o <= $outputs; $o++) {
    $name = "Output";
    if ($outputs > 1) { $name .= " $o"; } // Output doesn't have a name in the MIB, add number if >1
...
foreach ($cache['apc'] as $index => $entry) {
  if ($apc_unit_map[$entry['coolingUnitExtendedAnalogUnits']] && isset($entry['coolingUnitExtendedAnalogValue'])) {
    ...
    if ($entry['coolingUnitExtendedAnalogUnits'] === 'kW' ||
      ...
      $scale = 1000 / $entry['coolingUnitExtendedAnalogUnits'];
    } elseif ($entry['coolingUnitExtendedAnalogUnits'] === 'hr') {
      $scale = 3600 / $entry['coolingUnitExtendedAnalogUnits'];
    } elseif ($entry['coolingUnitExtendedAnalogUnits'] === 'weeks') {
      $scale = 604800 / $entry['coolingUnitExtendedAnalogUnits'];
    } else {
      $scale = 1 / $entry['coolingUnitExtendedAnalogUnits'];
    }

...
$sentry3_InfeedEntry = snmpwalk_cache_twopart_oid($device, 'InfeedEntry', [], 'Sentry3-MIB');
$sentry3_OutletEntry = snmpwalk_cache_threepart_oid($device, 'OutletEntry', [], 'Sentry3-MIB');
...
foreach ($sentry3_InfeedEntry as $tower => $feeds) {
    foreach ($feeds as $feed => $entry) {
        ...
        if (isset($entry['infeedLoadValue']) && $entry['infeedLoadValue'] >= 0) {
            $limits = ['limit_high'      => $entry['infeedCapacity'],
        ...
        foreach ($sentry3_OutletEntry[$tower][$feed] as $outlet => $ou_entry) {
            ...
            if (isset($ou_entry['outletLoadValue']) && $ou_entry['outletLoadValue'] >= 0) {
                ...
                $limits = ['limit_high' => $ou_entry['outletLoadHighThresh'],
...
$sentry3_TempHumidSensorEntry = snmpwalk_cache_oid($device, 'TempHumidSensorEntry', [], 'Sentry3-MIB');
...
foreach ($sentry3_TempHumidSensorEntry as $index => $entry) {
    ...
    if (isset($entry['tempHumidSensorTempValue']) && $entry['tempHumidSensorTempValue'] >= 0) {
        ...
        $options = ['limit_high' => (isset($entry['tempHumidSensorTempHighThresh']) ? $entry['tempHumidSensorTempHighThresh'] * $scale_temp : NULL),
    ...
    if (isset($entry['tempHumidSensorHumidValue']) && $entry['tempHumidSensorHumidValue'] >= 0) {
        $options = ['limit_high' => (isset($entry['tempHumidSensorHumidHighThresh']) ? $entry['tempHumidSensorHumidHighThresh'] : NULL),

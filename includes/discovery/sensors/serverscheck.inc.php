...
foreach ($sensors_def as $entry)
{
  ...
  if ($value != '-')
  {
    ...
    if (is_numeric($value))
    {
      ...
      discover_status_ng($device, $mib, $entry['oid'], $oid_num, $index, 'serverscheck-status', $descr, $value, array('entPhysicalClass' => 'other'));
    }
  }
}
...
foreach ($inputs_def as $entry)
{
  ...
  if ($value != '-')
  {
    ...
    discover_status_ng($device, $mib, $entry['oid'], $oid_num, $index, 'serverscheck-input', $descr, $value, array('entPhysicalClass' => 'input'));
  }
}

...
$wlan_table = snmpwalk_cache_oid($device, 'RuckusWLANTable', [], 'RUCKUS-WLAN-MIB');

if (OBS_DEBUG > 1) {
    print_vars($wlan_table);
}
...
foreach ($wlan_table as $wlan_ifIndex => $wlan) {
    ...
    if ($wlan['ruckusWLANAdminStatus'] == 'down') {
        $wlan['wlan_admin_status'] = 0;
    } else {
        $wlan['wlan_admin_status'] = 1;
    }

...
$oids_array = [
  'alvarion_events'        => ['brzaccVLTotalTxEvents.0', 'brzaccVLTotalRxEvents.0',
  ...
  'alvarion_frames_errors' => ['brzaccVLFramesDelayedDueToSwRetry.0', 'brzaccVLFramesDropped.0',
                               'brzaccVLRecievedBadFrames.0', 'brzaccVLNoOfDuplicateFramesDiscarded.0',
                               'brzaccVLNoOfInternallyDiscardedMirCir.0'],
  'alvarion_errors'        => ['brzaccVLPhyErrors.0', 'brzaccVLCRCErrors.0'],
];
...
foreach ($oids_array as $graph => $oids) {
    ...
    if (!isset($graphs_db[$graph]) || $graphs_db[$graph] === TRUE) {
        $data = snmp_get_multi_oid($device, $oids, [], "ALVARION-DOT11-WLAN-MIB");
        $data = $data[0];

        $rrd_file   = str_replace('_', '-', $graph) . ".rrd";
        $rrd_create = '';
        foreach ($oids as $oid) {
            $oid_ds     = truncate(str_replace(['brzaccVL', '.0'], '', $oid), 19, '');

...
if (isset($agent_data['uptime'])) {
    [$agent_data['uptime']] = explode(' ', $agent_data['uptime']);
...
foreach (get_device_mibs_permitted($device) as $mib) // Check every MIB supported by the device
{
    if (isset($config['mibs'][$mib]['la'])) {
        ...
        if (discovery_check_requires($device, $la_def, $device, 'device')) {
            continue;
        }
        ...
        if (isset($la_def['type']) && $la_def['type'] === 'table') {
            ...
            $la      = snmpwalk_cache_oid($device, $la_def['oid_1min'], [], $mib);
...
if ($config['geocoding']['enable']) {
    ...
    if ($geo_detect) {
        ...
        if ($update_geo) {
            ...
            $update_geo = ['location_updated' => format_unixtime($config['time']['now'], 'Y-m-d G:i:s')]; // Increase updated time
            dbUpdate($update_geo, 'devices_locations', '`location_id` = ?', [$geo_db['location_id']]);
        } # end if $update_geo
    }
}

...
if (isset($FROM_OBSERVIUM) && $FROM_OBSERVIUM == true) {
    if($_SESSION['userlevel'] > 7) { $ENABLED = true; }
...
if(isset($vars['wmap_id'])) { $vars['mapname'] = dbFetchCell("SELECT `wmap_name` FROM `weathermaps` WHERE `wmap_id` = ?", array($vars['wmap_id'])); }

if(isset($vars['action'])) { $action = $vars['action']; }
if(isset($vars['mapname'])) { $mapname = $vars['mapname'];  /*$mapname = wm_editor_sanitize_conffile($mapname);*/ }
if(isset($vars['selected'])) { $selected = wm_editor_sanitize_selected($vars['selected']); }
...
else
{
    ...
    $editor_name = "/wmap/mapname=".$mapname."/";
    ...
    */
	...
	case 'show_config':
        ...
        */
        ...
        echo dbFetchCell("SELECT `wmap_config` FROM `weathermaps` WHERE `wmap_name` = ?", array($mapfile));
	...
	case "set_map_properties":
		...
		$map->htmloutputfile = wm_editor_sanitize_file($vars['map_htmlfile'],array("html") );
		$map->imageoutputfile = wm_editor_sanitize_file($vars['map_pngfile'],array("png","jpg","gif","jpeg"));
		...
		{
		    $map->background = wm_editor_sanitize_file(stripslashes($vars['map_bgfile']),array("png","jpg","gif","jpeg") );
		}
	...
	case 'set_map_style':
		...
		if(wm_editor_validate_one_of($vars['mapstyle_htmlstyle'],array('static','overlib'),false)) {
